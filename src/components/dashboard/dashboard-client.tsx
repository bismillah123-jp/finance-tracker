...
function DashboardInner({ user, profile, initialTransactions }: DashboardClientProps) {
  ...
  const monthlyTxs = transactions.filter((t) => { const d = new Date(t.date); return d.getMonth() === cm && d.getFullYear() === cy; });
  const totalBalance = transactions.filter(t=>t.type==="income").reduce((s,t)=>s+t.amount,0) - transactions.filter(t=>t.type==="expense").reduce((s,t)=>s+t.amount,0);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col w-full max-w-lg mx-auto relative app-shell">
      ...
      <header className="fixed top-0 left-1/2 -translate-x-1/2 w-full max-w-lg z-30 px-4 py-3 flex items-center justify-between"
        ...
        <div className="flex items-center gap-1">
          ...
          <motion.button whileTap={{ scale: 0.88 }} onClick={() => setShowNotif(!showNotif)}
            className="w-8 h-8 rounded-xl flex items-center justify-center transition-colors hover:bg-secondary relative">
      ...
      {/* Content — pb-28 prevents bottom nav overlap */}
...
export default function DashboardClient(props: DashboardClientProps) {
  return <NotificationProvider><DashboardInner {...props} /></NotificationProvider>;
}
